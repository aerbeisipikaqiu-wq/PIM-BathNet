...
## =================================================================
## 2. ablation test M2：Late Fusion Baseline
## =================================================================
...
class YOLOLateFusionBaseNet(nn.Module):
    def __init__(self):
        ...
        # The input dimension is changed to 128 (for images) + 4 (physical scalars) = 132
    ...
    def forward(self, img, phys_feats):
        # 1. Extracting features independently based on spatial flow
        x = self.stem(img)
        ...
        # 2. Tail feature rough assembly (Late Fusion)
...
class SARDataset(Dataset):
    def __init__(self, csv_dir, patch_dir, t_idx_list, supply_dir=None, is_train=False):
        ...
        # A. Loading original base transect data
        ...
        if self.is_train and len(self.df) > 0:
            ...
            if len(deep_samples) > 0:
                print(f"    💡 [Data Augmentation] Mixed-domain dynamic deep-water line: {area4_start_bound:.2f}m")
                print(f"    💡 Global deep water (Area 4 and Area 5) total: {len(deep_samples)} samples, performing standard double oversampling...")
...
def main():
    ...
    # Fix the designated survey line as the validation set
    ...
    print("\n================ 📐 Fixed Survey Line Alignment Report ================")
    print(f"📡 Fixed the designated 【Validation Set】 survey line IDs ({len(val_transects)} lines): {val_transects}")
    print(f"🏋️ Automatically generated 【Training Set】 base survey line IDs ({len(train_transects)} lines): {train_transects}")
    print("========================================================")
    ...
    # Establish a global benchmark division line based on the training set
    ...
    print(f"\n🔥 Ablation Group M2 Activated: 【Late Fusion with End-Point Feature Concatenation + Standard Global MSE Control】")
    print("\n================ Starting Fixed Survey Line Isolation Training (100 Epochs) ================")
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        for imgs, phys, targets in train_loader:
            ...
            # Simultaneously, both the image and the physical vector are passed through for forward propagation.
        ...
        if epoch_val_loss < best_val_loss:
            ...
            print(f" -> 🎉 【Ablation Group M2】 validation set loss refreshed, weights secured.")
        else:
            patience_counter += 1
    ...
    print("\n================ 🏁 Training phase completed, initiating comprehensive error evaluation of all validation set survey lines ================")
    ...
    if len(val_dataset) > 0:
        ...
        print("-" * 150)
        print("📊 【M2 Ablation Group】: Average analysis of inversion errors for all validation set profiles at different water depth sections")
        print("-" * 150)
        print(f"{'Water Deep Level':<10}\t{'Corresponding Reference Water Depth Range':<20}\t{'Number of Regional Samples':<12}\t{'Average Absolute Error (MAE)':<20}\t{'Root Mean Square Error (RMSE)':<20}\t{'Average Relative Error (RE)':<20}\t{'Coefficient of Determination (R2)'}") 
        print("-" * 150)
        ...
        print("⚠️ Note: No valid full-section data for the validation set was obtained." )
