...
## =================================================================
## 2. 🌟 Ablation experiment M1: Pure YOLO spatial flow backbone network (Base) 🌟
## =================================================================
...
class YOLOSpaceStreamBaseNet(nn.Module):
    def __init__(self):
        ...
        # Pure Image Feature Extraction Backbone (Does Not Accept Physical Features at All)
        ...
        # Directly outputting an integrated linear regression head
    ...
    def forward(self, img):
        # Pure spatial flow feature transfer
        x = self.stem(img)
...
class SARDataset(Dataset):
    def __init__(self, csv_dir, patch_dir, t_idx_list, supply_dir=None, is_train=False):
        ...
        # A. Load the original basic survey line data
        ...
        if self.is_train and supply_dir and os.path.exists(supply_dir):
            supply_dfs = []
            print(f"\n📥 [Data Integration] Detected supplementary data source for the training set and is currently scanning. {supply_dir} ...")
            ...
            if len(supply_dfs) > 0:
                ...
                print('  ⚠️ Warning: No CSV file that conforms to the naming rule was found in the "supply_data" folder.')
                self.df = base_df
        else:
            self.df = base_df
        ...
        if self.is_train and len(self.df) > 0:
            ...
            if len(deep_samples) > 0:
                print(f"    💡 [Data Augmentation] Hybrid Domain Dynamic Deepwater Line: {area4_start_bound:.2f}m")
                print(f"    💡 Global deep water (Regions 4 and 5) total: {len(deep_samples)} samples, performing standard double over-sampling...")
    ...
    def __getitem__(self, idx):
        ...
        if img is None:
            raise FileNotFoundError(f"❌ Image patch reading failed! Please check the path: {img_path}")
...
## =================================================================
## 4. Main control flow and standard verification engine
## =================================================================
...
def main():
    ...
    print(f"⚡ Computing device locked: 【{device}】")
    ...
    # Base model: Pure YOLO spatial flow backbone network (Base) + Standard global MSE control
    ...
    print(f"\n🔥 Ablation group M1 activation: 【Pure YOLO spatial flow backbone Base + Standard global MSE control】】")
    print("\n================ Start fixed measurement line isolation and replacement process (100 Epochs) ================")
    
    for epoch in range(1, EPOCHS + 1):
        model.train()
        train_loss = 0.0
        for imgs, _, targets in train_loader:  # Discard physical input
            imgs, targets = imgs.to(device), targets.to(device)
            optimizer.zero_grad()
            
            # Forward propagation only takes in the image as input
            outputs = model(imgs)
            
            # Standard Global MSE Loss
        ...
        with torch.no_grad(): 
            for imgs, _, targets in val_loader:  # Discard physical input
                imgs, targets = imgs.to(device), targets.to(device)
                outputs = model(imgs)
        ...
        if epoch_val_loss < best_val_loss:
            ...
            print(f" -> 🎉 【Ablation group M1】 validation set loss refreshed record, weights secured.")
        else:
            patience_counter += 1
            
        if patience_counter >= early_stop_patience:
            print(f"\n🛑 [Early Stopping] Continuous {early_stop_patience} rounds without improvement, early termination.")
            break
            
    print("\n================ 🏁 Training phase completed, initiating comprehensive error evaluation across all validation lines ================")
    ...
    if len(val_dataset) > 0:
        ...
        print("-" * 145)
        print(" 📊【M1 Ablation group: Comprehensive error analysis across all validation lines】")
        print("-" * 145)
        print(f"{'Water Depth Level':<10}\t{'Corresponding Baseline Depth Range':<20}\t{'Regional Sample Count':<12}\t{'Average Absolute Error (MAE)':<20}\t{'Root Mean Square Error (RMSE)':<20}\t{'Average Relative Error (RE)':<20}\t{'Determination Coefficient (R2)'}")
        print("-" * 145)
        ...
        for i in range(5):
            ...
            print(f"region {i+1:<4}\t{low_b:5.1f}m ~ {high_b:5.1f}m\t\t{regional_count:<12}\t{mae_str:<20}\t{rmse_str:<20}\t{re_str:<20}\t{r2_str}")
            
        print("=" * 145)
    else:
        print("⚠️Warning: No valid full-section data for the validation set has been obtained.。")
