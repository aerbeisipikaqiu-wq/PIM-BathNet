...
## =================================================================
## 2. 🌟 消融实验 M3 独有核心组件：多级锚定与仅通道调制器 🌟
## =================================================================

class PhysAnchorMLP(nn.Module):
    """
    多级尺度锚定器：将一维的4维物理特征映射为不同层级所需的控制通道数
    """
...
class PhyInNetM3(nn.Module):
    ...
    def forward(self, img, phys_feats):
        ...
        x = self.mod1(x, p1)  # Stage 1 仅通道调制
        
        x = self.stage2_c2f(self.stage2_conv(x))
        x = self.mod2(x, p2)  # Stage 2 仅通道调制
        
        x = self.stage3_c2f(self.stage3_conv(x))
        x = self.mod3(x, p3)  # Stage 3 仅通道调制
...
def main():
    ...
    print(f"\n🔥 消融组 M3 激活: 【PhyIn-Net 仅通道缩放变体 + 标准全局 MSE 控制】")
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        # 🌟 用于收集当前 Epoch 所有的真实值与预测值，计算全局 R2
        epoch_true_list = []
        epoch_pred_list = []
        ...
        if epoch_val_loss < best_val_loss:
            ...
            print(f" -> 🎉 【消融组 M3】验证集损失刷新纪录，权重安全锁定。")
        else:
            patience_counter += 1
    ...
    if len(val_dataset) > 0:
        ...
        # 🌟 调整打印输出面板的横线宽度以匹配新增的 R2 列
        print("-" * 145)
        print(" 📊【M3 消融组：全部验证集测线不同水深段反演误差平均分析】")
        print("-" * 145)
